...
def process_dataset(
    ...
    for idx, img_path in enumerate(images, 1):
        try:
            ...
            w, h = img.size
            print(f"[{idx:2d}/{len(images)}] {img_path.name:20s} ({w:4d}x{h:4d}) ", end="", flush=True)
    ...
    if processed > 0:
        ...
        print(f"   lora-trainer --config examples/config_fern_test.yaml \\")
        print(f"                --dataset {output_dir} \\")
        print(f"                --run-dir ./runs/test_fern")
